...
export async function searchGo(
    ...
    try {
        ...
        if (apiResponse.status === 200) {
            ...
            newTerm = await generateResponse(
                client,
                promptGoApi,
                modelIdentifier
            );
            ...
            elizaLogger.info(`new term: ${newTerm}, old term: ${term}`);
        } else {
            elizaLogger.info(
                `EBI API gave response code ${apiResponse.status}`
            );
        }
    } catch (error) {
...
export async function searchAtc(
    ...
    try {
        ...
        if (apiResponse.status === 200) {
            ...
            elizaLogger.error(
                `ATC API gave response code ${apiResponse.status}`
            );
        }
    } catch (error) {
        elizaLogger.error(`Error generating response: ${error}`);
