...
export async function downloadPaperAndExtractDOI(paperUrl: string) {
    ...
    if (paperUrl.includes("biorxiv.org")) {
        ...
        elizaLogger.error(
            "Unsupported URL. Only bioRxiv and arXiv URLs are supported."
        );
        return;
    }
    ...
    try {
        ...
        if (doi) {
            elizaLogger.info("DOI:", doi);
        } else {
            elizaLogger.info("DOI not found using any method.");
        }
        ...
        elizaLogger.info(`PDF file downloaded successfully as "${fileName}".`);
        return { pdfBuffer, doi };
    } catch (error) {
        elizaLogger.error("An error occurred during the process:", error);
